...
# Route to setup a super user when the application is first launched as a new instance
def setup_superuser(request):
    user = get_user_model()

    # Seeing if a user has already been created.
    # If so, redirect to the normal registration form. We only want one superuser by default.
